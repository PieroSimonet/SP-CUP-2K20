...
while not(bagFile.LastTimeDone())
    ...
    if ~isempty(anomaly.peaks)
        ...
        for i = 1:length(anomaly.peaks)

            treesManager = treesManager.PushData(data);
            treesManager = treesManager.PushPeaksData(anomaly.peaks(i,:));
            isError = treesManager.SearchTree(@peaksWrapper);

            if(isError)
                disp('tree anomaly');
                disp(anomaly.peaks{i}); 
                break;
            end
        end
